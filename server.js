...
const handleApiRequest = async (req, res, url) => {
    if (req.method === 'OPTIONS' && url.pathname.startsWith('/api/')) {
        res.writeHead(204, buildCorsHeaders(CORS_PRESET_HEADERS));
        res.end();
        return true;
    }
    ...
    if (req.method === 'POST' && url.pathname === '/api/leads') {
        try {
            ...
            sendJson(res, 201, { success: true }, CORS_PRESET_HEADERS);
        } catch (error) {
            console.error('Erro ao registrar lead:', error);
            const status = error.statusCode && Number.isInteger(error.statusCode) ? error.statusCode : 500;
            sendJson(res, status, { success: false, message: error.message || 'Erro interno ao salvar o contato.' }, CORS_PRESET_HEADERS);
        }
    ...
    if (req.method === 'GET' && url.pathname === '/api/leads') {
        try {
            const leads = await readLeadsFromCsv();
            sendJson(res, 200, { leads }, CORS_PRESET_HEADERS);
        } catch (error) {
            console.error('Erro ao ler leads:', error);
            sendJson(res, 500, { success: false, message: 'Erro ao carregar os contatos.' }, CORS_PRESET_HEADERS);
        }
    ...
    if (req.method === 'GET' && url.pathname === '/api/leads.csv') {
        try {
            ...
            res.writeHead(200, {
                ...
                'Cache-Control': 'no-store',
                ...buildCorsHeaders(CORS_PRESET_HEADERS),
            });
            res.end(content);
        } catch (error) {
            console.error('Erro ao enviar CSV:', error);
            res.writeHead(500, buildCorsHeaders({ 'Content-Type': 'text/plain; charset=utf-8' }));
